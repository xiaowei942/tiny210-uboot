...
void s3c_usb_init_phy(void)
{
	...
	DBG_SETUP0("USB_PHY_CONTROL: 0x%x-0x%x\n",USB_PHY_CONTROL,USB_PHY_CONTROL);
	DBG_SETUP0("CLK_GATE_IP1_REG: 0x%x\n",CLK_GATE_IP1_REG);
	DBG_SETUP0("CLK_GATE_BLOCK_REG: 0x%x\n",CLK_GATE_BLOCK_REG);
...
void s3c_usb_phy_off(void) {
	////////////////  EDIT BY WEI  /////////////////
	writel(readl(S5P_OTG_PHYPWR)|(0x18), S5P_OTG_PHYPWR);//OTG block power down,Analog block power down
	writel(readl(USB_PHY_CONTROL)&~(1<<0), USB_PHY_CONTROL);//USB PHY0(OTG device) disable
...
void s3c_usb_core_soft_reset(void)
{
	...
	do
	{
		...
		///////////////  ADD BY WEI  /////////////////
		DBG_SETUP0("AHB_MASTER_IDLE: 0x%x and S5P_OTG_GRSTCTL: 0x%x\n",AHB_MASTER_IDLE,S5P_OTG_GRSTCTL);
		///////////////  END OF ADD  /////////////////
